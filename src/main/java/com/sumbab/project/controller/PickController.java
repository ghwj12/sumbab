...
import com.sumbab.project.pick.PickService;
...
public class PickController {
	...
	@RequestMapping(value = "Store/StoreView/Pick/PickProcess/{store_num}")
	public String memberPick(@PathVariable int store_num, HttpServletResponse response, HttpServletRequest request, Model model) {
		String id="";			//merge하면 session에 있는 id 사용
		int result=0;
		if(!id.equals("")) {		//로그인 했을 경우
			result = pickService.addPick(id, store_num);
			...
			if(value.equals("")) {						//보관함에 해당하는 쿠키가 존재하지 않을 경우
				...
				Cookie cookie = new Cookie("pick", Integer.toString(store_num));
				...
				for(int i=0; i<picked.length; i++) {
					if(picked[i].equals(Integer.toString(store_num))) {	//보관함에 추가할 가게번호가 이미 있을 경우
				...
				if(result != 1) {							//보관함에 추가할 가게번호가 없을 경우
					...
					list.add(Integer.toString(store_num));	//가게 번호 추가
					String finalStorage = "";				
